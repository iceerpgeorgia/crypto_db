...
export default async function CounteragentHistoryPage({ params }: { params: { id: string } }) {
  const prisma = new PrismaClient();
  const idNum = Number(params.id);
  const logs = await prisma.auditLog.findMany({
    where: { table: "counteragents", recordId: BigInt(idNum) },
  ...
  return (
    <div className="mx-auto max-w-[800px] px-6 py-8">
      <h1 className="text-2xl font-semibold mb-4">Counteragent History #{idNum}</h1>
      {logs.length === 0 ? (
